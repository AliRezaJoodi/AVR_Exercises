...
_0x0:
	.DB  0x54,0x43,0x28,0x75,0x56,0x29,0x3A,0x0
...
_main:
; .FSTART _main
; 0000 0027 int16_t tc_buf = 0;
; 0000 0028 int16_t tc_buf_last = 0;
; 0000 0029 int32_t tc_mv, tc_uv;
; 0000 002A int16_t tc_temp = 0;
; 0000 002B int16_t cj_temp = 50;
; 0000 002C int32_t cj_uv = 0;
; 0000 002D 
; 0000 002E uint8_t lcd_flag = 0;
; 0000 002F 
; 0000 0030 LCD_Config();
	SBIW R28,15
	LDI  R30,LOW(0)
	ST   Y,R30
	STD  Y+1,R30
...
; 0000 0033 while (1){
_0x6:
; 0000 0034 tc_buf = read_adc(TC_CH);
	LDI  R26,LOW(0)
	RCALL _read_adc
	MOVW R16,R30
; 0000 0035 if(tc_buf != tc_buf_last){
	__CPWRR 18,19,16,17
	BREQ _0x9
; 0000 0036 tc_buf_last = tc_buf;
	MOVW R18,R16
; 0000 0037 tc_mv = ((uint32_t)tc_buf * ADC_VREF) >> ADC_RESOLUTION_BIT;
	MOVW R26,R16
	CALL __CWD2
	__GETD1N 0x1388
	CALL __MULD12U
	...
	CALL __LSRD12
	__PUTD1S 11
; 0000 0038 tc_uv = (tc_mv * 1000) / TC_GAIN;
	__GETD2N 0x3E8
	CALL __MULD12
	MOVW R26,R30
	MOVW R24,R22
	...
	CALL __DIVD21
	__PUTD1S 7
; 0000 0039 cj_uv = TC_ConvertTempToMicroVolt(cj_temp);
	LDD  R26,Y+5
	LDD  R27,Y+5+1
	RCALL _TC_ConvertTempToMicroVolt
	__PUTD1S 1
; 0000 003A tc_temp = TC_ConvertMicroVoltToTemp(tc_uv + cj_uv);
	CALL SUBOPT_0x0
	CALL __ADDD21
	RCALL _TC_ConvertMicroVoltToTemp
	MOVW R20,R30
; 0000 003B 
; 0000 003C lcd_flag = 1;
	LDI  R30,LOW(1)
	ST   Y,R30
; 0000 003D }
; 0000 003E 
; 0000 003F if(lcd_flag){
_0x9:
	LD   R30,Y
	CPI  R30,0
	BREQ _0xA
; 0000 0040 lcd_flag = 0;
	LDI  R30,LOW(0)
	ST   Y,R30
; 0000 0041 LCD_DisplayMainPage(tc_uv + cj_uv, tc_temp);
	CALL SUBOPT_0x0
	CALL __ADDD12
	CALL __PUTPARD1
	MOVW R26,R20
...
; .FEND
;void LCD_DisplayMainPage(int32_t uv, int16_t temp){
; 0000 0047 void LCD_DisplayMainPage(int32_t uv, int16_t temp){
_LCD_DisplayMainPage:
; .FSTART _LCD_DisplayMainPage
; 0000 0048 char txt[6];
; 0000 0049 
; 0000 004A ltoa(uv, txt);
	ST   -Y,R27
	ST   -Y,R26
	SBIW R28,6
;	uv -> Y+8
...
; 0000 004B lcd_gotoxy(0,0);
	...
	LDI  R26,LOW(0)
	CALL _lcd_gotoxy
; 0000 004C lcd_putsf("TC(uV):");
	__POINTW2FN _0x0,0
	CALL SUBOPT_0x1
; 0000 004D lcd_puts(txt);
; 0000 004E lcd_putsf("  ");
; 0000 004F 
; 0000 0050 itoa(temp, txt);
	LDD  R30,Y+6
	LDD  R31,Y+6+1
	ST   -Y,R31
	ST   -Y,R30
...
; 0000 0051 lcd_gotoxy(0,1);
	...
	LDI  R26,LOW(1)
	CALL _lcd_gotoxy
; 0000 0052 lcd_putsf("Temp(C):");
	__POINTW2FN _0x0,11
	CALL SUBOPT_0x1
; 0000 0053 lcd_puts(txt);
; 0000 0054 lcd_putsf("  ");
; 0000 0055 }
	JMP  _0x20C0003
; .FEND
;void LCD_Config(void){
; 0000 006E void LCD_Config(void){
_LCD_Config:
; .FSTART _LCD_Config
...
; .FEND
;void ADC_Init(void){
; 0000 0074 void ADC_Init(void){
_ADC_Init:
; .FSTART _ADC_Init
; 0000 0075 // ADC initialization
; 0000 0076 // ADC Clock frequency: 125.000 kHz
; 0000 0077 // ADC Voltage Reference: AVCC pin
; 0000 0078 // ADC Auto Trigger Source: ADC Stopped
; 0000 0079 ADMUX=ADC_VREF_TYPE;
	LDI  R30,LOW(64)
	OUT  0x7,R30
; 0000 007A ADCSRA=(1<<ADEN) | (0<<ADSC) | (0<<ADATE) | (0<<ADIF) | (0<<ADIE) | (1<<ADPS2) | ...
	LDI  R30,LOW(134)
	OUT  0x6,R30
; 0000 007B SFIOR=(0<<ADTS2) | (0<<ADTS1) | (0<<ADTS0);
	LDI  R30,LOW(0)
	OUT  0x30,R30
; 0000 007C }
	RET
...
; 0001 00B2 int32_t uv0, uv1;
; 0001 00B3 int32_t t0;
; 0001 00B4 int32_t result;
; 0001 00B5 
; 0001 00B6 if(uv <= TC_K_Table10C[0]){
	CALL __PUTPARD2
	SBIW R28,16
	ST   -Y,R17
	ST   -Y,R16
...
;	result -> Y+2
	...
	CALL __CPD12
	BRLT _0x20003
; 0001 00B7 return TC_K_TEMP_MIN;
	LDI  R30,LOW(65266)
	LDI  R31,HIGH(65266)
	RJMP _0x20C0005
; 0001 00B8 }
; 0001 00B9 
; 0001 00BA if(uv >= TC_K_Table10C[TC_K_TABLE_SIZE-1]){
_0x20003:
	__POINTW1FN _TC_K_Table10C_G001,656
	CALL SUBOPT_0x2
	CALL __CPD21
	BRLT _0x20004
; 0001 00BB return TC_K_TEMP_MAX;
	LDI  R30,LOW(1370)
	LDI  R31,HIGH(1370)
	RJMP _0x20C0005
; 0001 00BC }
; 0001 00BD 
; 0001 00BE for(i = 0; i < (TC_K_TABLE_SIZE-1); i++){
_0x20004:
	__GETWRN 16,17,0
_0x20006:
	__CPWRN 16,17,164
	BRLT PC+2
	RJMP _0x20007
; 0001 00BF if(uv < TC_K_Table10C[i+1]){
	CALL SUBOPT_0x3
	CALL SUBOPT_0x2
	CALL __CPD21
	BRLT PC+2
	RJMP _0x20008
; 0001 00C0 uv0 = TC_K_Table10C[i];
	CALL SUBOPT_0x4
	__PUTD1S 14
; 0001 00C1 uv1 = TC_K_Table10C[i+1];
	CALL SUBOPT_0x3
	CALL __GETD1PF
	__PUTD1S 10
; 0001 00C2 
; 0001 00C3 t0 = TC_K_TEMP_MIN + ((int32_t)i * 10);
	MOVW R26,R16
	CALL __CWD2
	CALL SUBOPT_0x5
	CALL __MULD12
	...
	__PUTD1S 6
; 0001 00C4 
; 0001 00C5 result = (t0 + (((uv - uv0) * 10) / (uv1 - uv0)));
	__GETD2S 14
	__GETD1S 18
	CALL __SUBD12
	__GETD2N 0xA
	...
	CALL __ADDD12
	CALL SUBOPT_0x6
; 0001 00C6 return (int16_t)result;
	LDD  R30,Y+2
	LDD  R31,Y+2+1
	RJMP _0x20C0005
; 0001 00C7 }
...
; 0001 00CE int32_t TC_ConvertTempToMicroVolt(int16_t temp){
_TC_ConvertTempToMicroVolt:
; .FSTART _TC_ConvertTempToMicroVolt
; 0001 00CF int16_t index;
; 0001 00D0 int16_t t0;
; 0001 00D1 int16_t dt;
; 0001 00D2 int32_t uv0;
; 0001 00D3 int32_t uv1;
; 0001 00D4 int32_t duv;
; 0001 00D5 int32_t result;
; 0001 00D6 
; 0001 00D7 // Clamp temperature to table limits (-270°C to +1370°C)
; 0001 00D8 if (temp <= -270){
	ST   -Y,R27
	ST   -Y,R26
	SBIW R28,16
	CALL __SAVELOCR6
...
;	result -> Y+6
	...
	CPC  R31,R27
	BRLT _0x20009
; 0001 00D9 return TC_K_Table10C[0];
	LDI  R30,LOW(_TC_K_Table10C_G001*2)
	LDI  R31,HIGH(_TC_K_Table10C_G001*2)
	CALL __GETD1PF
	RJMP _0x20C0004
...
_0x20009:
	...
	CPC  R27,R30
	BRLT _0x2000A
; 0001 00DD return TC_K_Table10C[TC_K_TABLE_SIZE - 1];
	__POINTW1FN _TC_K_Table10C_G001,656
	CALL __GETD1PF
	RJMP _0x20C0004
; 0001 00DE }
; 0001 00DF 
; 0001 00E0 // Compute table index (each entry represents 10°C)
; 0001 00E1 index = (temp + 270) / 10;
_0x2000A:
	LDD  R26,Y+22
	LDD  R27,Y+22+1
	SUBI R26,LOW(-270)
	...
	MOVW R16,R30
; 0001 00E2 
; 0001 00E3 // Base temperature for this table cell
; 0001 00E4 t0 = (index * 10) - 270;
	LDI  R26,LOW(10)
	LDI  R27,HIGH(10)
	CALL __MULW12
	SUBI R30,LOW(270)
	...
	MOVW R18,R30
; 0001 00E5 
; 0001 00E6 // Lookup microvolt values
; 0001 00E7 uv0 = TC_K_Table10C[index];       // µV at t0
	CALL SUBOPT_0x4
	__PUTD1S 18
; 0001 00E8 uv1 = TC_K_Table10C[index + 1];   // µV at t0 + 10°C
	CALL SUBOPT_0x3
	CALL __GETD1PF
	__PUTD1S 14
; 0001 00E9 
; 0001 00EA // If temperature exactly matches a 10°C grid point
; 0001 00EB if (temp == t0){
	LDD  R26,Y+22
	LDD  R27,Y+22+1
	CP   R18,R26
	CPC  R19,R27
	BRNE _0x2000B
; 0001 00EC return uv0;
	__GETD1S 18
	RJMP _0x20C0004
; 0001 00ED }
; 0001 00EE 
; 0001 00EF // Linear interpolation (integer math, no floats)
; 0001 00F0 dt  = temp - t0;   // offset from base temperature (0..10)
_0x2000B:
	LDD  R30,Y+22
	LDD  R31,Y+22+1
	SUB  R30,R18
	SBC  R31,R19
	MOVW R20,R30
; 0001 00F1 duv = uv1 - uv0;   // delta µV between table entries
	__GETD2S 18
	__GETD1S 14
	CALL __SUBD12
	__PUTD1S 10
; 0001 00F2 
; 0001 00F3 // Return interpolated microvolt value
; 0001 00F4 result = uv0 + ((duv * dt) / 10);
	MOVW R30,R20
	__GETD2S 10
	CALL __CWD1
	CALL __MULD12
